...
export function Editor({ target, onClose }: {
  ...
  /**
   * The sheet's own "close, asking first if anything would be lost".
   *
   * Hardware back is handled by the Modal, out here, while whether there is
   * anything to lose is known only inside the sheet. The ref is how the two
   * meet without lifting the whole draft up a level.
   */
  const closeSafelyRef = useRef<() => void>(onClose);
  return (
    <Modal
      visible={target !== null}
      animationType="slide"
      transparent
      onRequestClose={() => closeSafelyRef.current()}
    ...
    >
      {target ? <Sheet target={target} onClose={onClose} closeSafelyRef={closeSafelyRef} /> : null}
    </Modal>
  );
}

function Sheet({ target, onClose, closeSafelyRef }: {
  target: EditorTarget;
  onClose: () => void;
  closeSafelyRef: React.MutableRefObject<() => void>;
}) {
  const p = useTheme();
  ...
  /**
   * WHAT THE SHEET OPENED HOLDING, so it can tell whether anything was typed.
   *
   * Tapping the dimmed area behind the sheet, or pressing hardware back, used
   * to discard a half-written event without a word. Back is the worse of the
   * two, because it is also the gesture for "go up a level" everywhere else in
   * this app, so it gets pressed out of habit.
   *
   * Compared by value, not by reference: `set()` rebuilds the draft on every
   * keystroke, so a reference test would call an untouched sheet dirty.
   */
  const openedWith = useRef<string>('');
  useEffect(() => { openedWith.current = JSON.stringify(draft); }, []);
  const isDirty = () => JSON.stringify(draft) !== openedWith.current;

  /**
   * Close, asking first if there is anything to lose.
   *
   * Used by both the scrim and hardware back, so the two cannot disagree about
   * what closing means.
   */
  const closeSafely = () => {
    ...
    Alert.alert(
      'Discard changes?',
      'What you have typed here will not be saved.',
      [
  ...
  closeSafelyRef.current = closeSafely;
  ...
  /**
   * Delete, at the scope the user chooses.
   *
   * A REPEAT OFFERED ONLY "the whole series". The long-press menu on the very
   * same item has offered the three proper scopes for a long time, so the
   * destructive path with fewer options was the one reached by the more obvious
   * route, and there was no way at all to drop a single occurrence from inside
   * the editor you were already standing in.
   *
   * The scopes and their wording match the menu's, and both go through
   * `applyScoped`, so "this one" means the same thing wherever it is pressed.
   */
  const runDelete = (scope: OccurrenceScope) => void (async () => {
    try {
      if (draft.recur) await applyScoped(target.store as SyncStore, target.id!, target.date, scope, 'delete');
      else await removeItem(target.store as SyncStore, target.id!);
      onClose();
    } catch (err) {
  ...
  const confirmDelete = () => {
    ...
    Alert.alert(
      'Delete which?',
      'This repeats. Choose how much of it to remove.',
      [
        { text: 'Cancel', style: 'cancel' },
        { text: 'This one only', onPress: () => runDelete('one') },
        { text: 'This and the rest', onPress: () => runDelete('following') },
        { text: 'The whole series', style: 'destructive', onPress: () => runDelete('all') },
      ],
    );
  };
  ...
  return (
    <View style={{ flex: 1, backgroundColor: p.scrim }}>
      ...
      <Pressable
        unstable_pressDelay={TAP_DELAY} style={StyleSheet.absoluteFill} onPress={closeSafely} accessibilityLabel="Close" />
